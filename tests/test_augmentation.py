...
from conftest import NEEDS_SIM_DATA
...
def test_mhs_geometry():
    """
    Assert that coordinate transformation function for MHS viewing
    geometry are reversible.
    """
    i = np.arange(0, 90, 10)
    j = np.arange(0, 90, 10)
...
@NEEDS_SIM_DATA
def test_swath_geometry(training_files_3d_mhs_sim):
    """
    Assert that coordinate transformation function for MHS viewing
    geometry are reversible.
    """
    input_data = decompress_and_load(training_files_3d_mhs_sim[0])
...
@NEEDS_SIM_DATA
def test_transformation_coordinates(training_files_3d_mhs_sim):
    """
