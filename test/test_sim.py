...
def test_match_l1c_amsr2():
    ...
    sim_path = Path(".").parent / "data" / "amsr2" / "sim"
    sim_file = SimFile(sim_path / "AMSR2.dbsatTb.20190101.027510.sim")
...
def test_process_sim_file_gmi_era5():
    ...
    subset = SubsetConfig(ocean_only=True)
    data = process_sim_file(sim_file,
                            sensors.GMI,
                            "ERA5",
                            era5_path,
...
def test_process_sim_file_tmi():
    DATA_PATH = Path(__file__).parent / "data"
    sim_file = DATA_PATH / "tmi" / "sim" / "TMIPR.dbsatTb.20190101.027510.sim"
    era5_path = "/qdata2/archive/ERA5/"
...
@pytest.mark.skipif(not HAS_ARCHIVES, reason="Data archives not available.")
def test_process_sim_file_ssmi():
    DATA_PATH = Path(__file__).parent / "data"
    sim_file = DATA_PATH / "ssmi" / "sim" / "SSMI.dbsatTb.20190101.027510.sim"
    era5_path = "/qdata2/archive/ERA5/"
    data = process_sim_file(sim_file, sensors.SSMI, "ERA5", era5_path)
...
@pytest.mark.skipif(not HAS_ARCHIVES, reason="Data archives not available.")
def test_process_l1c_file_ssmi():
    DATA_PATH = Path(__file__).parent / "data"
    l1c_file = (
        DATA_PATH /
        "ssmi" /
...
@pytest.mark.skipif(not HAS_ARCHIVES, reason="Data archives not available.")
def test_process_l1c_file_amsr2():
    l1c_file = (
        Path(__file__).parent /
        "data" /
        "amsr2" /
        "l1c" /
        "1C.GCOMW1.AMSR2.XCAL2016-V.20181001-S000409-E014301.033894.ITE753.HDF5"
    )
    era5_path = "/qdata2/archive/ERA5/"
